...
// swell instead of water's choppy waves). Reuses ggx.frag's SSBO/UBO layout,
...
// Mesh.cpp), even though lava has no texture maps — declared but unused so
...
// -------------------------------------------------------
// Lava look (set once per Material)
// -------------------------------------------------------
uniform float uTime;
uniform vec3  uRockColor;      // cooled crust
uniform vec3  uCrackColor;     // warm rim around a crack, not yet emissive
uniform vec3  uHotColor;       // emissive core of a crack
...
float fbm(vec2 p)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        freq *= 2.02; // slightly off 2.0 so octaves don't align into a grid
        amp  *= 0.5;
    }
    return sum;
}
...
void main()
{
    // Low-frequency domain warp so the crack pattern crawls/breathes instead
    // of just sliding linearly — a plain UV scroll reads as an obvious loop.
    vec2 flow = vec2(
        valueNoise(vWorldPos.xy * 0.04 + uTime * 0.07),
        valueNoise(vWorldPos.xy * 0.04 + 19.3 - uTime * 0.055)
    ) * 2.0 - 1.0;

    vec2 uv = vWorldPos.xy * 0.09 + flow * 0.9 + vec2(uTime * 0.05, -uTime * 0.035);
    float n = fbm(uv);

    // Ridge transform turns the noise field into thin glowing veins instead
    // of soft blobs — peaks where fbm crosses 0.5.
    float ridge = 1.0 - abs(n * 2.0 - 1.0);
    ridge = pow(clamp(ridge, 0.0, 1.0), 3.0);

    // Slow, spatially-offset pulse so cracks glow in and out of sync with
    // each other instead of the whole floor breathing as one flat block —
    // reads as heat rippling through the flow rather than a static texture.
    float pulse = 0.75 + 0.35 * sin(uTime * 1.6 + n * 9.0 + vWorldPos.x * 0.15 + vWorldPos.y * 0.11);

    vec3 albedo   = mix(uRockColor, uCrackColor, clamp(ridge * 1.4, 0.0, 1.0));
    vec3 emissive = uHotColor * ridge * uEmissiveStrength * pulse;

    // Bump detail from a higher-frequency noise layer, scrolled fast enough
    // to read as the surface actively churning rather than a static crust.
    vec2  buv = vWorldPos.xy * 0.6 + uTime * 0.08;
    const float e = 0.08;
    float h  = valueNoise(buv);
    float hx = valueNoise(buv + vec2(e, 0.0));
    float hy = valueNoise(buv + vec2(0.0, e));
    vec2  grad = vec2(hx - h, hy - h) / e;
    vec3  bump = normalize(vec3(-grad * 0.35, 1.0));
